...
Claim functions lock the build, resolve an identical idempotency key first, lock the plan, package, and account in the global build → plan → package → account order, validate the immutable revision/account/package launch snapshot, capture one database clock value, expire stale claims against that boundary, and insert the claim plus Gate 1 logical-resource intents before returning. A different active claim returns SQLSTATE `55P03`. SQL `NULL` provider outcomes fail closed rather than bypassing the accepted outcome vocabulary. Ambiguous outcomes enter `reconciliation_required`; they never become retryable mutation claims until readback records the resource as proven missing.
...
- An approved plan must transition to `draft` or `cancelled`, atomically cancelling and auditing its exact `pending_gate_1` build, before budget release. The transition captures plan status plus the exact matching build ID/count in one initial MVCC statement. An initially approved snapshot without exactly one matching build is invariant SQLSTATE `55000`; a non-approved snapshot or any later change is stale SQLSTATE `40001`. A valid captured build is then locked before the plan and revalidated. Direct release from `approved` is rejected.
- A failed state alone authorizes only read-only reconciliation: `gate_1_failed` and `qa_failed` admit Gate 1 reconciliation, while `gate_2_failed` admits Gate 2 reconciliation. No failed state by itself permits a create, retry, or delivery shortcut. The separate proof-gated retry RPC may mutate from `gate_1_failed` only when its supplied parent is each selected resource's latest create/retry mutation and the latest newer, successful, intent-scoped reconciliation proves every selected resource missing.
- A partial retry may complete successfully while the build remains `gate_1_failed`. Gate 1 readiness is calculated over every persisted logical build resource using intent-scoped, resource-bound causal evidence after that resource's latest mutation; the latest required result for every field must match, and every resource requires a provider ID and at least one authoritative required field before the build becomes `ready_to_deliver`.
...
The hardening verification additionally covers claim-versus-cancellation lock ordering in both winner orders, approval renewal and expiry recovery, failed-state reconciliation, full-intent subset-retry completion, build-wide claim exclusion, attributed ambiguity audit, immutable handoff identity, and complete-resource generic database lifecycles for Google, Meta, and TikTok (including extensible Google asset-group/asset resource types). A deterministic table-lock interleaving also proves that a real claim committed between an approved transition's initial read and lock acquisition is classified stale rather than as an invariant failure; exact child markers, SQLSTATEs, messages, result IDs, and native exits are checked independently of the final database state.
