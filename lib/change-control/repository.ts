...
export async function getMockChangeRequest(id: string): Promise<M03ChangeRequestDetail> {
  ...
  const [requests, items, revisions, validations, approvals, events, sourceVerifications, baselines, resourceMappings, attempts] = await Promise.all([
...
export async function recordM03ItemAttempt(input: {
  ...
  result: "provider_execution_locked" | "pending" | "succeeded" | "failed";
