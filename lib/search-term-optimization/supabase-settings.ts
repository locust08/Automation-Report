...
type Row = { google_customer_id:string;automation_enabled:boolean;schedule_frequency:AnalysisScheduleFrequency;auto_safe_score_threshold:number;high_spend_threshold:number;minimum_clicks_threshold:number;last_run_at:string|null;next_run_at:string|null };
const map=(r:Row):SearchTermAccountSettings=>({googleCustomerId:r.google_customer_id,automationEnabled:Boolean(r.automation_enabled),scheduleFrequency:r.schedule_frequency,autoSafeScoreThreshold:r.auto_safe_score_threshold,highSpendThreshold:Number(r.high_spend_threshold),minimumClicksThreshold:r.minimum_clicks_threshold,lastRunAt:r.last_run_at,nextRunAt:r.next_run_at});
