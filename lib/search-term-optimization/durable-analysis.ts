...
export function toClientJob(job:DurableAnalysisJob){const currentRunTerms=job.current_run>0?Math.min(SEARCH_TERMS_PER_RUN,Math.max(0,job.total_terms-(job.current_run-1)*SEARCH_TERMS_PER_RUN)):0;return{jobId:job.id,accountId:job.google_customer_id,status:job.status,stage:job.stage,startedAt:job.started_at??job.created_at,updatedAt:job.updated_at,heartbeatAt:job.last_worker_ping_at,totalTerms:job.total_terms,plannedRuns:job.planned_runs,currentRun:job.current_run,completedRuns:job.completed_runs,currentRunTerms,currentBatch:job.current_run,completedBatches:job.completed_runs,maxBatches:job.planned_runs,currentBatchSize:currentRunTerms,termsProcessed:job.terms_processed,retryCount:job.retry_count,error:job.error,progressComplete:job.status==="completed"};}
