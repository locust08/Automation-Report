...
export async function listSearchTermDecisionSummaryRows(date?:string):Promise<SearchTermDecisionSummaryRow[]>{
 const dateFilter=date?`&reviewed_at=gte.${qs(`${date}T00:00:00+08:00`)}&reviewed_at=lt.${qs(`${nextDate(date)}T00:00:00+08:00`)}`:"";
