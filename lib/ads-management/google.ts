...
export async function fetchManagedSearchCampaigns(accountId: string, dates?: { startDate?: string; endDate?: string }): Promise<{ campaigns: ManagedCampaign[]; synchronizedAt: string }> {
  ...
  const adPerformanceQuery = `SELECT campaign.id, ad_group.id, ad_group_ad.ad.id, metrics.cost_micros, metrics.impressions, metrics.clicks, metrics.conversions, metrics.interactions FROM ad_group_ad WHERE segments.date ${dateCondition} AND campaign.status != 'REMOVED' AND ad_group.status != 'REMOVED' AND ad_group_ad.status != 'REMOVED'`;
  ...
  for (const row of adRows) {
    ...
    group.ads.push({ id: ad.id, resourceName, name, status: row.adGroupAd?.status || "UNSPECIFIED", adType, adStrength: row.adGroupAd?.adStrength || "UNSPECIFIED", actionItems: row.adGroupAd?.actionItems ?? [], fields });
  }
  ...
  for (const row of adPerformanceRows) {
    ...
    if (ad) ad.performanceMetrics = managedPerformanceMetrics(row);
  }
