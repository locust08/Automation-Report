...
export async function saveContentSuitabilitySnapshot(payload:ContentSuitabilityPayload){await supabaseRest("ad_automation_content_suitability_snapshots?on_conflict=google_customer_id",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:jsonBody({google_customer_id:payload.account.customerId,customer_name:payload.account.customerName,payload_json:payload as unknown as Record<string, unknown>,refreshed_at:payload.refreshedAt})});}
