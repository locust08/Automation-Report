...
export function loadPlacementDashboard(input: { customerId: string; customerName: string; startDate: string; endDate: string; refreshedAt: string; warnings?: string[]; performanceMaxCampaignCount?: number }): PlacementDashboardPayload {
  ...
  try {
    ...
    const totalImpressions=rows.reduce((sum,row)=>sum+row.impressions,0);const totalSpend=rows.reduce((sum,row)=>sum+row.spend,0);const topSites=websiteRows.slice(0,5).map(({id,displayName,placement,targetUrl,campaignName,campaignType,impressions})=>({id,displayName,placement,targetUrl,campaignName,campaignType,impressions}));
    return { placementStorage:{status:"available"},account:{customerId:input.customerId,customerName:input.customerName,startDate:input.startDate,endDate:input.endDate,refreshedAt:input.refreshedAt},summary:{total:rows.length,needsReview:rows.filter(r=>r.reviewStatus==="pending_optimizer").length,awaitingApproval:rows.filter(r=>r.reviewStatus==="ready_for_approval").length,kept:rows.filter(r=>r.reviewStatus==="kept").length,kiv:rows.filter(r=>r.reviewStatus==="kiv").length,approved:rows.filter(r=>r.reviewStatus==="ready_for_publishing").length,rejected:rows.filter(r=>r.reviewStatus==="approver_rejected").length},performanceMax:{available:rows.length>0,campaignCount,totalImpressions,uniqueSites:new Set(websiteRows.map((row)=>row.placement)).size,topSites},campaignTypes:[{channelType:"PERFORMANCE_MAX",label:"Performance Max",campaignCount,placementCount:rows.length,impressions:totalImpressions,spend:totalSpend,available:rows.length>0}],placementOverview:{campaignCount,placementCount:rows.length,totalImpressions,totalSpend,uniqueSites:new Set(websiteRows.map(row=>row.placement)).size,topSites},rows,changeSets:[],reports:[],warnings:input.warnings ?? [] };
  } finally { db.close(); }
}
