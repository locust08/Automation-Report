...
export function AutomaticActionsPanel({role,dashboard}:{role:AuthRole;dashboard:OptimizationDashboardPayload}) {
  ...
  return <section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-3">
      ...
      <div className="flex items-center justify-between rounded-xl border bg-neutral-50 p-3"><div><p className="text-sm font-semibold">Enable for account</p><p className="mt-1 text-xs text-neutral-500">Dry-run until the global switch is live.</p></div><Switch disabled={role!=="admin"} checked={settings.automaticExclusionEnabled} onCheckedChange={value=>setSettings(current=>({...current,automaticExclusionEnabled:value}))}/></div>
    </div>
