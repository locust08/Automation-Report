...
export function ChangeControlPageClient({ initialRole }: { initialRole: AuthRole }) {
  ...
  useEffect(() => {
    if (hydratedDeepLink.current || searchParams.get("open") !== "new" || searchParams.get("platform") !== "google") return;
    hydratedDeepLink.current = true;
    const entityType = searchParams.get("entity_type") || "campaign";
    const entityIdentity = searchParams.get("entity_identity") || searchParams.get("campaign_identity") || "";
    setDetail(null); setProviderPreview(null); setEditing(null); setFormOpen(true);
    setForm({ ...emptyForm(), platform: "google", account_identity: searchParams.get("account_identity") || "", campaign_identity: searchParams.get("campaign_identity") || "", title: searchParams.get("title") || "Google Ads change request", reason: searchParams.get("reason") || "", items: [{ ...emptyItem(), entity_type: entityType, entity_identity: entityIdentity, field_path: searchParams.get("field_path") || "" }] });
  ...
  return <ReportShell title="Change Control" dateLabel="M03 Provider-Ready Workflow" reportReady initialRole={initialRole}>
    <div className="space-y-5">
      ...
      <LegacyGoogleHistory initialAccountId={searchParams.get("legacy_account_id") || ""} initialRequestId={searchParams.get("legacy_request_id") || ""} />
...
function RequestEditor({ form, setForm, editing, busy, onClose, onSave }: { form: RequestForm; setForm: React.Dispatch<React.SetStateAction<RequestForm>>; editing: M03ChangeRequestSummary | null; busy: boolean; onClose: () => void; onSave: () => void }) {
  ...
  return <Card className="bg-white"><CardHeader className="border-b"><div className="flex items-start justify-between"><div><CardTitle>{editing ? "Edit change request" : "New change request"}</CardTitle><CardDescription>{editing ? "Only draft requests can be edited; identity stays fixed." : "Create a reviewed provider-ready request. Nothing is sent while execution is locked."}</CardDescription></div><Button size="icon" variant="ghost" onClick={onClose}><XIcon /><span className="sr-only">Close</span></Button></div></CardHeader><CardContent className="space-y-5">
    ...
    {form.platform === "google" && !editing ? <GoogleResourcePicker accountIdentity={form.account_identity} campaignIdentity={form.campaign_identity} onSelect={(resource) => {
      setForm((current) => ({ ...current, campaign_identity: resource.campaign_id, items: current.items.map((item, index) => index === 0 ? { ...item, entity_type: resource.type, entity_identity: resource.id, platform_resource_mapping: { ...item.platform_resource_mapping, customer_id: current.account_identity, campaign_id: resource.campaign_id, ...(resource.type === "ad_group" ? { ad_group_id: resource.id } : {}), ...(resource.type === "ad" && resource.parent_id ? { ad_group_id: resource.parent_id, ad_id: resource.id } : {}) } } : item) }));
    }} /> : null}
    ...
    {form.platform === "tiktok" && !editing ? <TikTokResourcePicker accountIdentity={form.account_identity} campaignIdentity={form.campaign_identity} onSelect={(resource) => {
      setForm((current) => ({ ...current, campaign_identity: resource.type === "campaign" ? resource.id : current.campaign_identity, items: current.items.map((item, index) => index === 0 ? { ...item, entity_type: resource.type === "ad_group" ? "ad_group" : resource.type, entity_identity: resource.id, platform_resource_mapping: { ...item.platform_resource_mapping, advertiser_id: current.account_identity, ...(resource.type === "ad_group" ? { adgroup_id: resource.id } : {}), ...(resource.type === "ad" && resource.parent_id ? { adgroup_id: resource.parent_id } : {}), ...(resource.type === "identity" ? { identity_id: resource.id } : {}), ...(resource.type === "video" ? { video_id: resource.id } : {}), ...(resource.type === "pixel" ? { pixel_code: resource.id } : {}) } } : item) }));
    }} /> : null}
    <div className="space-y-3"><div className="flex items-center justify-between"><h3 className="font-semibold">Field changes</h3><Button size="sm" variant="outline" onClick={() => setForm((current) => ({ ...current, items: [...current.items, emptyItem()] }))}><PlusIcon /> Add field</Button></div>{form.items.map((item, index) => <div key={index} className="grid gap-3 rounded-xl border bg-slate-50 p-4 md:grid-cols-2 lg:grid-cols-4">
      <Field label="Entity"><Input className="bg-white" value={item.entity_type} onChange={(event) => updateItem(index, { entity_type: event.target.value })} /></Field><Field label="Entity identity"><Input className="bg-white" value={item.entity_identity} onChange={(event) => updateItem(index, { entity_identity: event.target.value })} /></Field><Field label="Field path">{form.platform === "google" ? <select className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={item.field_path} onChange={(event) => updateItem(index, { field_path: event.target.value, value_type: googleFieldValueType(event.target.value) })}><option value="">Select a supported Google field</option>{GOOGLE_FIELD_OPTIONS.map(([group, fields]) => <optgroup key={group} label={group}>{fields.map((field) => <option key={field} value={field}>{field}</option>)}</optgroup>)}</select> : form.platform === "meta" ? <select className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={item.field_path} onChange={(event) => updateItem(index, { field_path: event.target.value, value_type: metaFieldValueType(event.target.value) })}><option value="">Select a supported Meta field</option>{META_FIELD_OPTIONS.map(([group, fields]) => <optgroup key={group} label={group}>{fields.map((field) => <option key={field} value={field}>{field}</option>)}</optgroup>)}</select> : <select className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={item.field_path} onChange={(event) => updateItem(index, { field_path: event.target.value, value_type: tiktokFieldValueType(event.target.value) })}><option value="">Select a supported TikTok field</option>{TIKTOK_FIELD_OPTIONS.map(([group, fields]) => <optgroup key={group} label={group}>{fields.map((field) => <option key={field} value={field}>{field}</option>)}</optgroup>)}</select>}</Field><Field label="Value type"><select value={item.value_type} onChange={(event) => updateItem(index, { value_type: event.target.value as M03ChangeItemInput["value_type"] })} className="h-9 w-full rounded-md border bg-white px-3 text-sm"><option>string</option><option>number</option><option>boolean</option><option>json</option><option>null</option></select></Field><div className="lg:col-span-2"><Field label="Baseline value"><Textarea className="h-24 resize-none bg-white" value={String(item.baseline_value ?? "")} onChange={(event) => updateItem(index, { baseline_value: event.target.value })} /></Field></div><div className="lg:col-span-2"><Field label="Proposed value"><Textarea className="h-24 resize-none bg-white" value={String(item.proposed_value ?? "")} onChange={(event) => updateItem(index, { proposed_value: event.target.value })} /></Field></div>{form.platform === "meta" && (item.field_path.startsWith("ad.copy.") || item.field_path.startsWith("ad.creative.")) ? <div className="grid gap-3 md:col-span-2 lg:col-span-4 md:grid-cols-3"><Field label="Meta ad set ID"><Input className="bg-white" value={String(item.platform_resource_mapping?.ad_set_id ?? "")} onChange={(event) => updateItem(index, { platform_resource_mapping: { ...item.platform_resource_mapping, account_id: form.account_identity.replace(/^act_/, ""), ad_set_id: event.target.value } })} /></Field><Field label="Facebook Page ID"><Input className="bg-white" value={String(item.platform_resource_mapping?.page_id ?? "")} onChange={(event) => updateItem(index, { platform_resource_mapping: { ...item.platform_resource_mapping, page_id: event.target.value } })} /></Field><Field label="Instagram identity ID (optional)"><Input className="bg-white" value={String(item.platform_resource_mapping?.instagram_actor_id ?? "")} onChange={(event) => updateItem(index, { platform_resource_mapping: { ...item.platform_resource_mapping, instagram_actor_id: event.target.value } })} /></Field></div> : null}{form.platform === "tiktok" && (item.field_path.startsWith("ad.copy.") || item.field_path.startsWith("ad.creative.")) ? <div className="grid gap-3 md:col-span-2 lg:col-span-4 md:grid-cols-4"><Field label="TikTok ad group ID"><Input className="bg-white" value={String(item.platform_resource_mapping?.adgroup_id ?? "")} onChange={(event) => updateItem(index, { platform_resource_mapping: { ...item.platform_resource_mapping, advertiser_id: form.account_identity, adgroup_id: event.target.value } })} /></Field><Field label="Identity ID"><Input className="bg-white" value={String(item.platform_resource_mapping?.identity_id ?? "")} onChange={(event) => updateItem(index, { platform_resource_mapping: { ...item.platform_resource_mapping, identity_id: event.target.value } })} /></Field><Field label="Video ID"><Input className="bg-white" value={String(item.platform_resource_mapping?.video_id ?? "")} onChange={(event) => updateItem(index, { platform_resource_mapping: { ...item.platform_resource_mapping, video_id: event.target.value } })} /></Field><Field label="Approved final status"><select className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={String(item.platform_resource_mapping?.intended_status ?? "DISABLE")} onChange={(event) => updateItem(index, { platform_resource_mapping: { ...item.platform_resource_mapping, creative_mode: "REGULAR", intended_status: event.target.value } })}><option value="DISABLE">Disabled</option><option value="ENABLE">Enabled</option></select></Field></div> : null}{form.items.length > 1 ? <Button variant="ghost" className="text-red-700" onClick={() => setForm((current) => ({ ...current, items: current.items.filter((_, itemIndex) => itemIndex !== index) }))}>Remove field</Button> : null}
    </div>)}</div>
...
function LegacyGoogleHistory({ initialAccountId, initialRequestId }: { initialAccountId: string; initialRequestId: string }) {
  ...
  async function loadLegacy() {
    setBusy(true); setError(null);
    try {
      const payload = await api<{ requests?: LegacyGoogleRequest[]; changeSets?: LegacyGoogleRequest[] }>(`/api/ads-management/change-requests?accountId=${encodeURIComponent(accountId)}`);
  ...
  async function selectLegacy(id: string) {
    ...
    setBusy(true); setError(null);
    try { setSelected(await api<LegacyGoogleDetail>(`/api/ads-management/change-requests/${id}`)); }
    catch (caught) { setError(message(caught)); } finally { setBusy(false); }
  }
  useEffect(() => {
...
function GoogleResourcePicker({ accountIdentity, campaignIdentity, onSelect }: { accountIdentity: string; campaignIdentity: string; onSelect: (resource: GoogleSynchronizedResource) => void }) {
  ...
  async function loadResources() {
    ...
    try {
      ...
      const payload = await api<{ resources: GoogleSynchronizedResource[] }>(`/api/change-control/google/resources?${query}`);
      setResources(payload.resources);
    } catch (caught) { setError(message(caught)); } finally { setBusy(false); }
  }
  return <div className="rounded-xl border border-red-200 bg-red-50/40 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">Synchronized Google resources</h3><p className="text-sm text-muted-foreground">Select the exact official campaign, ad group, or ad. Discovery is read-only.</p></div><Badge variant="outline">Provider mutations locked</Badge></div><div className="mt-4 grid gap-3 md:grid-cols-[160px_1fr_1fr_auto]"><Select value={type} onValueChange={(value) => setType(value as GoogleSynchronizedResourceType)}><SelectTrigger className="bg-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="campaign">Campaigns</SelectItem><SelectItem value="ad_group">Ad groups</SelectItem><SelectItem value="ad">Ads</SelectItem></SelectContent></Select><Input className="bg-white" placeholder="Search name or ID" value={search} onChange={(event) => setSearch(event.target.value)} /><Input className="bg-white" disabled={type === "campaign"} placeholder={type === "ad" ? "Ad group or campaign ID" : "Campaign ID"} value={parentIdentity} onChange={(event) => setParentIdentity(event.target.value)} /><Button variant="outline" disabled={busy || !accountIdentity.trim()} onClick={() => void loadResources()}><RefreshCwIcon /> {busy ? "Loading" : "Load"}</Button></div>{error ? <p className="mt-3 text-sm text-red-700">{error}</p> : null}{resources.length ? <div className="mt-3 grid max-h-64 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">{resources.map((resource) => <button type="button" key={`${resource.type}:${resource.id}`} onClick={() => onSelect(resource)} className="rounded-lg border bg-white p-3 text-left hover:border-red-300 hover:bg-red-50"><span className="block font-medium">{resource.name}</span><span className="mt-1 block text-xs text-muted-foreground">{resource.type.replaceAll("_", " ")} · {resource.id} · {resource.status} · {resource.fields.length} editable fields</span></button>)}</div> : null}</div>;
}
...
function TikTokResourcePicker({ accountIdentity, campaignIdentity, onSelect }: { accountIdentity: string; campaignIdentity: string; onSelect: (resource: TikTokSynchronizedResource) => void }) {
  const [type, setType] = useState<TikTokSynchronizedResourceType>("campaign");
  const [parentIdentity, setParentIdentity] = useState("");
  const [search, setSearch] = useState("");
  const [resources, setResources] = useState<TikTokSynchronizedResource[]>([]);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  async function loadResources() {
    setBusy(true); setError(null);
    try {
      const query = new URLSearchParams({ account_identity: accountIdentity, type });
      if (search.trim()) query.set("search", search.trim());
      if ((type === "ad_group" || type === "ad") && (parentIdentity.trim() || campaignIdentity)) query.set("parent_identity", parentIdentity.trim() || campaignIdentity);
      const payload = await api<{ resources: TikTokSynchronizedResource[] }>(`/api/change-control/tiktok/resources?${query}`);
      setResources(payload.resources);
    } catch (caught) { setError(message(caught)); } finally { setBusy(false); }
  }
  return <div className="rounded-xl border border-red-200 bg-red-50/40 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">Synchronized TikTok resources</h3><p className="text-sm text-muted-foreground">Read official TikTok identities and select the exact regular Auction resource. This performs GET requests only.</p></div><Badge variant="outline">Provider mutations locked</Badge></div><div className="mt-4 grid gap-3 md:grid-cols-[160px_1fr_1fr_auto]"><select className="h-9 rounded-md border bg-white px-3 text-sm" value={type} onChange={(event) => setType(event.target.value as TikTokSynchronizedResourceType)}><option value="campaign">Campaigns</option><option value="ad_group">Ad groups</option><option value="ad">Ads</option><option value="identity">Identities</option><option value="video">Videos</option><option value="pixel">Pixels</option></select><Input className="bg-white" placeholder="Search name or ID" value={search} onChange={(event) => setSearch(event.target.value)} /><Input className="bg-white" disabled={!(["ad_group", "ad"] as string[]).includes(type)} placeholder={type === "ad" ? "Ad group ID (optional)" : "Campaign ID (optional)"} value={parentIdentity} onChange={(event) => setParentIdentity(event.target.value)} /><Button variant="outline" disabled={busy || !accountIdentity.trim()} onClick={() => void loadResources()}><RefreshCwIcon /> {busy ? "Loading" : "Load"}</Button></div>{error ? <p className="mt-3 text-sm text-red-700">{error}</p> : null}{resources.length ? <div className="mt-3 grid max-h-64 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">{resources.map((resource) => <button type="button" key={`${resource.type}:${resource.id}`} onClick={() => onSelect(resource)} className="rounded-lg border bg-white p-3 text-left hover:border-red-300 hover:bg-red-50"><span className="block font-medium">{resource.name}</span><span className="mt-1 block text-xs text-muted-foreground">{resource.type.replaceAll("_", " ")} · {resource.id}{resource.status ? ` · ${resource.status}` : ""}</span></button>)}</div> : null}</div>;
}
...
function googleFieldValueType(path: string): M03ChangeItemInput["value_type"] {
  if (path.includes("micros") || path.endsWith("target_roas")) return "number";
  if (path.includes("keywords") || path.includes("locations") || path.includes("placements") || path.includes("headlines") || path.includes("descriptions") || path.includes("final_urls") || path.includes("sitelinks")) return "json";
  return "string";
}

function tiktokFieldValueType(path: string): M03ChangeItemInput["value_type"] {
  if (path.includes("budget") || path.endsWith("bid.amount")) return "number";
