...
export function ChangeRequestPageClient({ id, currentUser, embedded = false, open = true, onClose, onProgressChange }: { id: string; currentUser: AuthenticatedAdsUser; embedded?: boolean; open?: boolean; onClose?: (keepAlive?: boolean) => void; onProgressChange?: (busy: boolean, status: string) => void }) {
  ...
  useEffect(() => { void load().catch((cause) => setError(formatAdsManagementUserError(cause, "Unable to load the change request. Please try again later."))); }, [load]);
  ...
  async function act(url: string, body: unknown = {}) {
    ...
    try {
      ...
      setError(formatAdsManagementUserError(cause, "Unable to complete this action. Please try again later."));
    } finally {
      polling = false;
  ...
  async function saveReviewContext() {
    ...
    try {
      ...
      setError(formatAdsManagementUserError(cause, "Unable to save the review details. Please try again later."));
    } finally {
      setBusy(false);
    }
  }
  ...
  const content = <div className={`${embedded ? "space-y-5 p-4 sm:p-5" : "mx-auto max-w-6xl space-y-5"}`}>
    <header className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap justify-between gap-4"><div><p className="text-sm text-slate-500">Change request · {data.status.replaceAll("_", " ")}</p><h1 className="text-2xl font-semibold">{data.title}</h1><p className="mt-1 text-sm text-slate-500">{data.account_name} · created by {data.created_by_name} · version {data.version}</p><p className="mt-1 text-xs text-slate-500">Signed in as {currentUser.displayName} · {adsRoleLabel(currentUser.role)}</p></div><div className="flex gap-2">{embedded ? <Button type="button" variant="outline" onClick={() => onClose?.(busy)}><XIcon />{busy ? "Continue in background" : "Close"}</Button> : <><Button asChild variant="outline"><Link href={`/manage?platform=google&accountId=${encodeURIComponent(data.account_id)}&accountName=${encodeURIComponent(data.account_name)}&view=change_requests`}>Manage account</Link></Button><Button asChild variant="outline"><Link href={`/manage/google/history?accountId=${data.account_id}&accountName=${encodeURIComponent(data.account_name)}`}>History</Link></Button></>}</div></div></header>
    ...
    {busy ? <section className="rounded-2xl border border-red-200 bg-red-50 p-4 text-red-900"><div className="flex items-center gap-3"><Loader2Icon className="size-5 animate-spin" /><div><strong className="block">{workflowProgressLabel(data.status)}</strong><span className="text-sm text-red-700">This panel updates automatically while Google processes the request.</span></div></div></section> : null}
    ...
    {data.approved_payload_hash ? <section className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-950"><strong>Approved immutable revision</strong><div className="mt-1 break-all font-mono text-xs">{data.approved_payload_hash}</div><div className="mt-1 text-xs">Approval expires {data.approval_expires_at ? new Date(data.approval_expires_at).toLocaleString() : "—"}</div></section> : null}
    ...
    {data.status === "awaiting_approval" ? <section className="rounded-2xl border border-red-200 bg-red-50 p-5"><h2 className="font-semibold text-red-950">2. Approve immutable revision</h2><p className="mt-1 text-sm text-red-800">Approval binds the current revision hash and Google preflight state for 24 hours. Any edit requires a new validation and approval.</p>{canEdit ? <Button className="mt-4" disabled={busy} onClick={() => void act(`/api/ads-management/change-requests/${id}/approve`, { comment: "Approved from Google Ads change review." })}>Approve revision</Button> : <RoleNotice text="Only administrators can approve Google Ads changes." />}</section> : null}
