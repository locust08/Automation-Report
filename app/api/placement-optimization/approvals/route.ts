...
import { savePlacementApproverDecision } from "@/lib/placement-optimization/sqlite-repository";
...
export async function POST(request: Request) {
  ...
  try {
    return NextResponse.json(savePlacementApproverDecision({ recommendationIds: ids, decision, reviewer: { id: session.sub, email: session.email, role: session.role } }));
  } catch (error) {
