...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) { try { const session = await authSessionFromRequest(request); if (!session) return NextResponse.json({ error: "Authentication required." }, { status: 401 }); if (!canEditAds(session.role)) return NextResponse.json({ error: "Your role cannot edit Google Ads change requests." }, { status: 403 }); const body = await request.json() as { version: number; reason?: string; changes: DraftChangeInput[]; editorContext?: DraftEditorContext }; return NextResponse.json(await replaceDraftChanges((await params).id, body.version, body.changes || [], sessionDisplayName(session), body.reason, body.editorContext)); } catch (error) { return NextResponse.json({ error: msg(error) }, { status: 409 }); } }
