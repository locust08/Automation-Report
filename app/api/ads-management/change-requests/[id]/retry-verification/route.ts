...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await params;
    return NextResponse.json({ error: "provider_execution_locked", message: "Provider verification is disabled until the future provider-integration phase." }, { status: 423 });
  } catch (error) {
