...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.completionMessage && body.completionMessage.length > 5000) return NextResponse.json({ error: "Completion message must be 5,000 characters or fewer." }, { status: 400 });
    return NextResponse.json(await publishChangeRequest((await params).id, workflowActorFromRequest(request, session), body.completionMessage));
  } catch (error) {
