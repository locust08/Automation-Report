...
export async function POST(request: Request) {
  ...
  try {
    ...
    const workerUrl=process.env.SEARCH_TERM_ANALYSIS_WORKER_URL?.replace(/\/$/,"");const workerSecret=process.env.WORKER_API_SECRET||process.env.REPORT_AUTOMATION_SECRET;
...
export async function PUT(request:Request){
  ...
  else{const child=spawn("doppler",["run","--",process.execPath,path.join(process.cwd(),"scripts","run-search-term-analysis-job.mjs"),job.id,job.google_customer_id,job.reporting_start_date??"",job.reporting_end_date??""],{cwd:process.cwd(),windowsHide:true,stdio:"ignore"});child.unref();}
