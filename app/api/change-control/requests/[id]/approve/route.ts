...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { return NextResponse.json(await approveMockChangeRequest((await params).id, parsed.data.idempotency_key, parsed.data.comment, { id: session.sub, name: session.fullName || session.email || "Administrator" })); }
  catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Approval failed." }, { status: error instanceof M03RepositoryError ? error.status : 500 }); }
}
