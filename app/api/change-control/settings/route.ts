...
export async function GET(request: Request) {
  ...
  try { buildTrustedRequestContext(request, session); return NextResponse.json(await getWorkflowSettings()); }
...
export async function PUT(request: Request) {
  ...
  try { return NextResponse.json(await updateWorkflowSetting(parsed.data, buildTrustedRequestContext(request, session))); }
  catch (error) { return failure(error); }
}
