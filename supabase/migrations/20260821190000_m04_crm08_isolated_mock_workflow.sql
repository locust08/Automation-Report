-- M04 CRM08 isolated mock-workflow contract. Additive: owns only public.m04_ads_* and m04_ads_internal.
-- M04 campaign planning and two-gate launch persistence contract.
create extension if not exists pgcrypto;
create schema if not exists m04_ads_internal;

create table public.m04_ads_ad_accounts (
...
create table public.m04_ads_budget_packages (
  id bigint generated always as identity primary key,
  client_id uuid not null,
...
create table public.m04_ads_campaign_plans (
  id bigint generated always as identity primary key,
  client_id uuid not null,
  budget_package_id bigint not null constraint acp_budget_package_fk references public.m04_ads_budget_packages(id) on delete restrict,
  ad_account_id bigint not null constraint acp_ad_account_fk references public.m04_ads_ad_accounts(id) on delete restrict,
...
create index m04_acp_package_idx on public.m04_ads_campaign_plans(budget_package_id);
create index m04_acp_account_idx on public.m04_ads_campaign_plans(ad_account_id);

create table public.m04_ads_campaign_plan_revisions (
  id bigint generated always as identity primary key,
  plan_id bigint not null constraint acpr_plan_fk references public.m04_ads_campaign_plans(id) on delete restrict,
  revision_number integer not null constraint acpr_number check (revision_number > 0),
  client_id uuid not null,
  ad_account_id bigint not null constraint acpr_account_fk references public.m04_ads_ad_accounts(id) on delete restrict,
  budget_package_id bigint not null constraint acpr_package_fk references public.m04_ads_budget_packages(id) on delete restrict,
...
create index m04_acpr_plan_idx on public.m04_ads_campaign_plan_revisions(plan_id, created_at desc);
create index m04_acpr_account_idx on public.m04_ads_campaign_plan_revisions(ad_account_id);
create index m04_acpr_package_idx on public.m04_ads_campaign_plan_revisions(budget_package_id);

alter table public.m04_ads_campaign_plans
  add constraint acp_active_revision_fk foreign key (active_revision_id) references public.m04_ads_campaign_plan_revisions(id) on delete restrict,
  add constraint acp_approved_revision_fk foreign key (approved_revision_id) references public.m04_ads_campaign_plan_revisions(id) on delete restrict,
  add constraint acp_reserved_revision_fk foreign key (reserved_revision_id) references public.m04_ads_campaign_plan_revisions(id) on delete restrict;
create index m04_acp_active_revision_idx on public.m04_ads_campaign_plans(active_revision_id);
create index m04_acp_approved_revision_idx on public.m04_ads_campaign_plans(approved_revision_id);
create index m04_acp_reserved_revision_idx on public.m04_ads_campaign_plans(reserved_revision_id);

create table public.m04_ads_campaign_approvals (
  id bigint generated always as identity primary key,
  plan_id bigint not null constraint aca_plan_fk references public.m04_ads_campaign_plans(id) on delete restrict,
  revision_id bigint not null constraint aca_revision_fk references public.m04_ads_campaign_plan_revisions(id) on delete restrict,
  ...
  comment text,
  superseded_approval_id bigint constraint aca_superseded_fk references public.m04_ads_campaign_approvals(id) on delete restrict,
...
create index m04_aca_plan_idx on public.m04_ads_campaign_approvals(plan_id, created_at desc);
create index m04_aca_revision_idx on public.m04_ads_campaign_approvals(revision_id);
create index m04_aca_superseded_idx on public.m04_ads_campaign_approvals(superseded_approval_id);

create table public.m04_ads_campaign_builds (
  id bigint generated always as identity primary key,
  plan_id bigint not null constraint acb_plan_fk references public.m04_ads_campaign_plans(id) on delete restrict,
  revision_id bigint not null constraint acb_revision_fk references public.m04_ads_campaign_plan_revisions(id) on delete restrict,
  revision_hash text not null constraint acb_hash check (revision_hash ~ '^[a-f0-9]{64}$'),
  approval_id bigint not null constraint acb_approval_fk references public.m04_ads_campaign_approvals(id) on delete restrict,
  budget_package_id bigint not null constraint acb_package_fk references public.m04_ads_budget_packages(id) on delete restrict,
  ad_account_id bigint not null constraint acb_account_fk references public.m04_ads_ad_accounts(id) on delete restrict,
...
create index m04_acb_plan_idx on public.m04_ads_campaign_builds(plan_id);
create index m04_acb_revision_idx on public.m04_ads_campaign_builds(revision_id);
create index m04_acb_approval_idx on public.m04_ads_campaign_builds(approval_id);
create index m04_acb_package_idx on public.m04_ads_campaign_builds(budget_package_id);
create index m04_acb_account_idx on public.m04_ads_campaign_builds(ad_account_id);

create table public.m04_ads_campaign_build_resources (
  id bigint generated always as identity primary key,
  build_id bigint not null constraint acbr_build_fk references public.m04_ads_campaign_builds(id) on delete restrict,
...
create index m04_acbr_build_idx on public.m04_ads_campaign_build_resources(build_id);

create table public.m04_ads_campaign_gate_attempts (
  id bigint generated always as identity primary key,
  build_id bigint not null constraint acga_build_fk references public.m04_ads_campaign_builds(id) on delete restrict,
  ...
  retry_parent_attempt_id bigint constraint acga_retry_parent_fk references public.m04_ads_campaign_gate_attempts(id) on delete restrict,
  attempt_number integer not null constraint acga_number check (attempt_number > 0),
  revision_id bigint not null constraint acga_revision_fk references public.m04_ads_campaign_plan_revisions(id) on delete restrict,
...
create index m04_acga_build_idx on public.m04_ads_campaign_gate_attempts(build_id);
create index m04_acga_retry_parent_idx on public.m04_ads_campaign_gate_attempts(retry_parent_attempt_id);
create index m04_acga_revision_idx on public.m04_ads_campaign_gate_attempts(revision_id);
create unique index m04_acga_build_gate_active_idx on public.m04_ads_campaign_gate_attempts(build_id, gate)
  where released_at is null and status = 'claimed';

create table public.m04_ads_campaign_qa_results (
  id bigint generated always as identity primary key,
  attempt_id bigint not null constraint acqr_attempt_fk references public.m04_ads_campaign_gate_attempts(id) on delete restrict,
  build_resource_id bigint constraint acqr_resource_fk references public.m04_ads_campaign_build_resources(id) on delete restrict,
...
create index m04_acqr_attempt_idx on public.m04_ads_campaign_qa_results(attempt_id);
create index m04_acqr_resource_idx on public.m04_ads_campaign_qa_results(build_resource_id);

create table public.m04_ads_campaign_audit_events (
  id bigint generated always as identity primary key,
  plan_id bigint constraint acaud_plan_fk references public.m04_ads_campaign_plans(id) on delete restrict,
  revision_id bigint constraint acaud_revision_fk references public.m04_ads_campaign_plan_revisions(id) on delete restrict,
  build_id bigint constraint acaud_build_fk references public.m04_ads_campaign_builds(id) on delete restrict,
  attempt_id bigint constraint acaud_attempt_fk references public.m04_ads_campaign_gate_attempts(id) on delete restrict,
...
create index m04_acaud_plan_idx on public.m04_ads_campaign_audit_events(plan_id, created_at desc);
create index m04_acaud_revision_idx on public.m04_ads_campaign_audit_events(revision_id, created_at desc);
create index m04_acaud_build_idx on public.m04_ads_campaign_audit_events(build_id, created_at desc);
create index m04_acaud_attempt_idx on public.m04_ads_campaign_audit_events(attempt_id, created_at desc);

create table public.m04_ads_campaign_monitoring_handoffs (
  id bigint generated always as identity primary key,
  build_id bigint not null constraint acmh_build_fk references public.m04_ads_campaign_builds(id) on delete restrict,
  client_id uuid not null,
  platform text not null constraint acmh_platform check (platform in ('google', 'meta', 'tiktok')),
  ad_account_id bigint not null constraint acmh_account_fk references public.m04_ads_ad_accounts(id) on delete restrict,
  provider_account_id text not null constraint acmh_provider_account check (btrim(provider_account_id) <> ''),
  revision_id bigint not null constraint acmh_revision_fk references public.m04_ads_campaign_plan_revisions(id) on delete restrict,
...
create unique index m04_acmh_build_uidx on public.m04_ads_campaign_monitoring_handoffs(build_id);
create index m04_acmh_revision_idx on public.m04_ads_campaign_monitoring_handoffs(revision_id);
create index m04_acmh_account_idx on public.m04_ads_campaign_monitoring_handoffs(ad_account_id);

create or replace function m04_ads_internal.is_approved_operator()
returns boolean
language sql
stable
security definer
...
create or replace function m04_ads_internal.reject_m04_row_mutation()
returns trigger
language plpgsql
set search_path = ''
as $$
...
create or replace function m04_ads_internal.is_nonempty_json_object(p_value jsonb)
returns boolean
language sql
immutable
set search_path = ''
...
create or replace function m04_ads_internal.protect_campaign_build_resource()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  ...
  if tg_op = 'INSERT' then
    if new.verified_at is not null
      and coalesce(pg_catalog.current_setting('m04_ads_internal.gate_2_finalizer', true), '') <> 'on' then
...
create or replace function m04_ads_internal.append_campaign_audit(
  p_plan_id bigint,
...
begin
  ...
  from m04_ads_internal.resolve_m04_actor(p_actor_id) as actor;
...
create or replace function public.m04_ads_create_campaign_plan_revision(
  p_plan_id bigint,
...
declare
  ...
  v_account public.m04_ads_ad_accounts%rowtype;
  v_package public.m04_ads_budget_packages%rowtype;
...
begin
  ...
  select plan.*
  into v_plan
  from public.m04_ads_campaign_plans as plan
  ...
  select account.*
  into v_account
  from public.m04_ads_ad_accounts as account
  where account.id = v_plan.ad_account_id;

  select package.*
  into v_package
  from public.m04_ads_budget_packages as package
  ...
  from public.m04_ads_campaign_plan_revisions as revision
  where revision.plan_id = v_plan.id;

  insert into public.m04_ads_campaign_plan_revisions (
  ...
  update public.m04_ads_campaign_plans
  ...
  perform m04_ads_internal.append_campaign_audit(
...
create or replace function public.m04_ads_reserve_campaign_budget(
  p_plan_id bigint,
  p_revision_id bigint,
...
returns public.m04_ads_campaign_plans
language plpgsql
security definer
set search_path = ''
as $$
...
begin
  ...
  update public.m04_ads_budget_packages
...
create or replace function public.m04_ads_release_campaign_budget(
  p_plan_id bigint,
  p_expected_plan_lock_version bigint,
  p_reason text,
  p_actor_id uuid,
...
create or replace function public.m04_ads_approve_campaign_plan_revision(
  p_plan_id bigint,
  p_revision_id bigint,
  p_expected_revision_hash text,
  p_expected_plan_lock_version bigint,
...
returns public.m04_ads_campaign_builds
language plpgsql
security definer
set search_path = ''
as $$
declare
  ...
  v_existing_approval public.m04_ads_campaign_approvals%rowtype;
...
begin
  ...
  from public.m04_ads_campaign_approvals as approval
  ...
  if found then
    ...
    select build.*
    into strict v_build
    from public.m04_ads_campaign_builds as build
...
create or replace function public.m04_ads_transition_campaign_plan(
  p_plan_id bigint,
...
create or replace function m04_ads_internal.assert_campaign_build_immutable_launch_snapshot(
  p_build public.m04_ads_campaign_builds,
  p_plan public.m04_ads_campaign_plans
)
returns void
language plpgsql
security definer
...
revoke all on function m04_ads_internal.assert_campaign_build_immutable_launch_snapshot(
  public.m04_ads_campaign_builds, public.m04_ads_campaign_plans
) from public, anon, authenticated, service_role;

create or replace function m04_ads_internal.assert_campaign_gate_2_manifest(
  p_build public.m04_ads_campaign_builds,
  p_action text,
  p_intent jsonb
)
...
begin
  ...
  if exists (
    ...
    where not exists (
      select 1
      from public.m04_ads_campaign_build_resources as resource
  ...
  if p_action = 'reconcile' then
    ...
    from public.m04_ads_campaign_gate_attempts as attempt
...
revoke all on function m04_ads_internal.assert_campaign_gate_2_manifest(
  public.m04_ads_campaign_builds, text, jsonb
) from public, anon, authenticated, service_role;

create or replace function m04_ads_internal.acquire_campaign_gate_claim_hardened(
  p_build_id bigint,
  p_gate smallint,
  p_action text,
  p_request_idempotency_key text,
...
returns public.m04_ads_campaign_gate_attempts
language plpgsql
security definer
set search_path = ''
as $$
declare
  ...
  v_plan public.m04_ads_campaign_plans%rowtype;
  v_approval public.m04_ads_campaign_approvals%rowtype;
  v_attempt public.m04_ads_campaign_gate_attempts%rowtype;
  v_actor_name text;
  v_actor_email text;
  v_actor_role text;
...
begin
  ...
  from public.m04_ads_campaign_builds as build where build.id = p_build_id for update;
  if not found then
  ...
  from public.m04_ads_campaign_plans as plan where plan.id = v_build.plan_id for update;
  ...
  perform m04_ads_internal.assert_campaign_build_immutable_launch_snapshot(v_build, v_plan);
  ...
  from public.m04_ads_campaign_approvals as approval where approval.id = v_build.approval_id;
  ...
  update public.m04_ads_campaign_gate_attempts
  ...
  if v_expired_count > 0 then
    update public.m04_ads_campaign_builds
  ...
  if exists (
    select 1 from public.m04_ads_campaign_gate_attempts as attempt
    where attempt.build_id = v_build.id and attempt.status = 'claimed'
      and attempt.released_at is null
  ) then
  ...
  if p_gate = 1 then
    ...
    if exists (
      select 1 from pg_catalog.jsonb_array_elements(p_intent -> 'resources') as item(value)
      join public.m04_ads_campaign_build_resources as resource
    ...
    perform m04_ads_internal.assert_campaign_gate_2_manifest(v_build, p_action, p_intent);
  end if;

  select coalesce(pg_catalog.max(attempt.attempt_number), 0) + 1
  into v_attempt_number from public.m04_ads_campaign_gate_attempts as attempt
  where attempt.build_id = v_build.id;
  ...
  if p_gate = 1 then
    insert into public.m04_ads_campaign_build_resources (build_id, logical_resource_key, resource_type)
...
create or replace function m04_ads_internal.acquire_campaign_retry_claim_hardened(
  p_build_id bigint,
...
declare
  ...
  v_plan public.m04_ads_campaign_plans%rowtype;
  v_approval public.m04_ads_campaign_approvals%rowtype;
  v_parent public.m04_ads_campaign_gate_attempts%rowtype;
  v_attempt public.m04_ads_campaign_gate_attempts%rowtype;
  v_actor_name text;
  v_actor_email text;
  v_actor_role text;
...
begin
  ...
  from public.m04_ads_campaign_gate_attempts as attempt where attempt.id = p_prior_attempt_id;
  if not found then
  ...
  if exists (
    ...
    where (
      select mutation.id
      from public.m04_ads_campaign_gate_attempts as mutation
  ...
  if exists (
    select 1
    from pg_catalog.jsonb_array_elements(p_intent -> 'resources') as item(value)
    left join public.m04_ads_campaign_build_resources as resource
      on resource.build_id = v_build.id
     and resource.logical_resource_key = item.value ->> 'logical_resource_key'
    where resource.id is null
      ...
      or (
        select evidence_attempt.status = 'succeeded'
          and exists (
            select 1 from public.m04_ads_campaign_qa_results as qa
            where qa.attempt_id = evidence_attempt.id
              ...
              and m04_ads_internal.is_nonempty_json_object(qa.readback_evidence)
          )
          and not exists (
            select 1
            from (
              ...
              from public.m04_ads_campaign_qa_results as qa
              where qa.attempt_id = evidence_attempt.id
                and qa.build_resource_id = resource.id
                and qa.phase = 'reconciliation' and qa.required
            ...
            where field_qa.result <> 'missing'
              or not m04_ads_internal.is_nonempty_json_object(field_qa.readback_evidence)
          )
        from public.m04_ads_campaign_gate_attempts as evidence_attempt
  ...
  from public.m04_ads_campaign_gate_attempts as attempt where attempt.build_id = v_build.id;
  insert into public.m04_ads_campaign_gate_attempts (
...
create or replace function public.m04_ads_acquire_campaign_gate_claim(
  p_build_id bigint,
  p_gate smallint,
  p_action text,
  p_request_idempotency_key text,
...
create or replace function public.m04_ads_acquire_campaign_retry_claim(
  p_build_id bigint,
  p_prior_attempt_id bigint,
  p_request_idempotency_key text,
  p_expected_revision_hash text,
...
begin
  ...
  select plan.* into strict v_plan from public.m04_ads_campaign_plans as plan where plan.id = v_build.plan_id;
  select approval.* into strict v_approval from public.m04_ads_campaign_approvals as approval where approval.id = v_build.approval_id;
  if v_approval.plan_id is distinct from v_build.plan_id
    or v_approval.revision_id is distinct from v_build.revision_id
    or v_approval.revision_hash is distinct from v_build.revision_hash then
    raise exception 'Campaign build approval revision lock does not match the build' using errcode = '55000';
        ...
        select evidence_attempt.status = 'succeeded'
          ...
          and not exists (
            ...
            where qa.attempt_id = evidence_attempt.id
              ...
              and qa.phase = 'reconciliation' and qa.required
              and (qa.result <> 'missing'
                or not m04_ads_internal.is_nonempty_json_object(qa.readback_evidence))
          )
        from public.m04_ads_campaign_gate_attempts as evidence_attempt
...
create or replace function public.m04_ads_record_campaign_resource_outcome(
  p_attempt_id bigint,
  p_claim_token uuid,
...
returns public.m04_ads_campaign_build_resources
language plpgsql
security definer
set search_path = ''
as $$
...
begin
  select attempt.* into v_attempt
  from public.m04_ads_campaign_gate_attempts as attempt where attempt.id = p_attempt_id;
  if not found then
    raise exception 'Gate attempt was not found' using errcode = 'P0002';
  end if;
  select build.* into strict v_build
  from public.m04_ads_campaign_builds as build where build.id = v_attempt.build_id for update;
  select attempt.* into strict v_attempt
  from public.m04_ads_campaign_gate_attempts as attempt where attempt.id = p_attempt_id for update;
  ...
  update public.m04_ads_campaign_build_resources
...
create or replace function m04_ads_internal.campaign_gate_2_manifest_ready(
  p_build_id bigint,
  p_attempt_id bigint,
  p_phase text
)
        ...
        where resource.id is null
          ...
          or exists (
            ...
            where latest_qa.id is null
              ...
              or not m04_ads_internal.is_nonempty_json_object(latest_qa.readback_evidence)
          )
      )
      and not exists (
        select 1
...
revoke all on function m04_ads_internal.campaign_gate_2_manifest_ready(
  bigint, bigint, text
) from public, anon, authenticated, service_role;

create or replace function public.m04_ads_append_campaign_qa_result(
  p_attempt_id bigint,
  p_claim_token uuid,
...
returns public.m04_ads_campaign_qa_results
language plpgsql
security definer
set search_path = ''
as $$
declare
  ...
  v_build public.m04_ads_campaign_builds%rowtype;
  v_resource public.m04_ads_campaign_build_resources%rowtype;
  v_result public.m04_ads_campaign_qa_results%rowtype;
...
begin
  ...
  if p_phase in ('gate_2', 'reconciliation')
    and not m04_ads_internal.is_nonempty_json_object(p_readback_evidence) then
...
create or replace function public.m04_ads_finalize_campaign_gate_claim(
  p_attempt_id bigint,
  p_claim_token uuid,
  ...
  if v_attempt.action = 'reconcile' then
    if v_attempt.gate = 1 then
      ...
      if p_provider_outcome in ('succeeded', 'missing')
        and m04_ads_internal.is_nonempty_json_object(p_final_readback_evidence)
      ...
      select
        m04_ads_internal.is_nonempty_json_object(p_final_readback_evidence)
        and exists (
          select 1
          from public.m04_ads_campaign_qa_results as qa
          join public.m04_ads_campaign_build_resources as resource on resource.id = qa.build_resource_id
      ...
      if v_gate_qa_ready then
        ...
        update public.m04_ads_campaign_build_resources as resource
        ...
        perform pg_catalog.set_config('m04_ads_internal.gate_2_finalizer', 'off', true);
      else
...
create or replace function public.m04_ads_create_campaign_monitoring_handoff(
  p_build_id bigint,
...
returns public.m04_ads_campaign_monitoring_handoffs
language plpgsql
security definer
set search_path = ''
as $$
declare
  v_build public.m04_ads_campaign_builds%rowtype;
  v_existing public.m04_ads_campaign_monitoring_handoffs%rowtype;
  v_handoff public.m04_ads_campaign_monitoring_handoffs%rowtype;
  v_revision public.m04_ads_campaign_plan_revisions%rowtype;
  v_account public.m04_ads_ad_accounts%rowtype;
  v_campaign public.m04_ads_campaign_build_resources%rowtype;
  v_actor_name text;
  v_actor_email text;
  v_actor_role text;
  v_child_ids jsonb;
begin
  ...
  from public.m04_ads_campaign_monitoring_handoffs as handoff where handoff.build_id = v_build.id;
  if found then
    return v_existing;
  end if;
  ...
  if v_build.status <> 'verified' or v_build.verified_at is null
    or not m04_ads_internal.is_nonempty_json_object(v_build.final_readback_evidence) then
  ...
  from public.m04_ads_campaign_plan_revisions as revision where revision.id = v_build.revision_id;
  select account.* into strict v_account
  from public.m04_ads_ad_accounts as account where account.id = v_build.ad_account_id;
...
create trigger acpr_append_only before update or delete on public.m04_ads_campaign_plan_revisions
for each row execute function m04_ads_internal.reject_m04_row_mutation();
create trigger aca_append_only before update or delete on public.m04_ads_campaign_approvals
for each row execute function m04_ads_internal.reject_m04_row_mutation();
create trigger acbr_protect before insert or update or delete on public.m04_ads_campaign_build_resources
for each row execute function m04_ads_internal.protect_campaign_build_resource();
create trigger acqr_append_only before update or delete on public.m04_ads_campaign_qa_results
for each row execute function m04_ads_internal.reject_m04_row_mutation();
create trigger acaud_append_only before update or delete on public.m04_ads_campaign_audit_events
for each row execute function m04_ads_internal.reject_m04_row_mutation();
create trigger acmh_append_only before update or delete on public.m04_ads_campaign_monitoring_handoffs
for each row execute function m04_ads_internal.reject_m04_row_mutation();

alter table public.m04_ads_ad_accounts enable row level security;
alter table public.m04_ads_budget_packages enable row level security;
alter table public.m04_ads_campaign_plans enable row level security;
alter table public.m04_ads_campaign_plan_revisions enable row level security;
alter table public.m04_ads_campaign_approvals enable row level security;
alter table public.m04_ads_campaign_builds enable row level security;
alter table public.m04_ads_campaign_build_resources enable row level security;
alter table public.m04_ads_campaign_gate_attempts enable row level security;
alter table public.m04_ads_campaign_qa_results enable row level security;
alter table public.m04_ads_campaign_audit_events enable row level security;
alter table public.m04_ads_campaign_monitoring_handoffs enable row level security;
...
begin
  foreach table_name in array array[
    'm04_ads_ad_accounts','m04_ads_budget_packages','m04_ads_campaign_plans','m04_ads_campaign_plan_revisions',
    'm04_ads_campaign_approvals','m04_ads_campaign_builds','m04_ads_campaign_build_resources',
    'm04_ads_campaign_gate_attempts','m04_ads_campaign_qa_results','m04_ads_campaign_audit_events',
    'm04_ads_campaign_monitoring_handoffs'
  ] loop
    execute format('revoke all on table public.%I from public, anon, authenticated, service_role', table_name);
    execute format('grant select on table public.%I to authenticated', table_name);
    execute format('grant select on table public.%I to service_role', table_name);
    execute format(
      'create policy %I on public.%I for select to authenticated using ((select m04_ads_internal.is_approved_operator()))',
...
revoke all on function m04_ads_internal.is_approved_operator() from public, anon;
revoke all on function m04_ads_internal.reject_m04_row_mutation() from public, anon, authenticated;
revoke all on function m04_ads_internal.is_nonempty_json_object(jsonb) from public, anon, authenticated, service_role;
revoke all on function m04_ads_internal.protect_campaign_build_resource() from public, anon, authenticated, service_role;
revoke all on function m04_ads_internal.resolve_m04_actor(uuid) from public, anon, authenticated, service_role;
revoke all on function m04_ads_internal.append_campaign_audit(bigint, bigint, bigint, bigint, text, text, text, uuid, inet, text, jsonb) from public, anon, authenticated, service_role;
grant usage on schema m04_ads_internal to authenticated, service_role;
grant execute on function m04_ads_internal.is_approved_operator() to authenticated, service_role;

revoke all on function public.m04_ads_create_campaign_plan_revision(bigint, bigint, jsonb, text, text, uuid, inet, text) from public, anon, authenticated, service_role;
revoke all on function public.m04_ads_reserve_campaign_budget(bigint, bigint, text, bigint, uuid, inet, text) from public, anon, authenticated, service_role;
revoke all on function public.m04_ads_release_campaign_budget(bigint, bigint, text, uuid, inet, text) from public, anon, authenticated, service_role;
revoke all on function public.m04_ads_approve_campaign_plan_revision(bigint, bigint, text, bigint, timestamptz, text, text, uuid, inet, text) from public, anon, authenticated, service_role;
revoke all on function public.m04_ads_transition_campaign_plan(bigint, bigint, text, text, text, uuid, inet, text) from public, anon, authenticated, service_role;
revoke all on function public.m04_ads_acquire_campaign_gate_claim(bigint, smallint, text, text, bigint, text, integer, jsonb, uuid, inet, text) from public, anon, authenticated, service_role;
revoke all on function public.m04_ads_acquire_campaign_retry_claim(bigint, bigint, text, text, integer, jsonb, uuid, inet, text) from public, anon, authenticated, service_role;
revoke all on function public.m04_ads_record_campaign_resource_outcome(bigint, uuid, text, text, text, text, jsonb, jsonb) from public, anon, authenticated, service_role;
revoke all on function public.m04_ads_append_campaign_qa_result(bigint, uuid, bigint, text, text, boolean, jsonb, jsonb, text, text, text, jsonb) from public, anon, authenticated, service_role;
revoke all on function public.m04_ads_finalize_campaign_gate_claim(bigint, uuid, text, jsonb, uuid, inet, text) from public, anon, authenticated, service_role;
revoke all on function public.m04_ads_create_campaign_monitoring_handoff(bigint, bigint, text, uuid, inet, text) from public, anon, authenticated, service_role;

grant execute on function public.m04_ads_create_campaign_plan_revision(bigint, bigint, jsonb, text, text, uuid, inet, text) to service_role;
grant execute on function public.m04_ads_reserve_campaign_budget(bigint, bigint, text, bigint, uuid, inet, text) to service_role;
grant execute on function public.m04_ads_release_campaign_budget(bigint, bigint, text, uuid, inet, text) to service_role;
grant execute on function public.m04_ads_approve_campaign_plan_revision(bigint, bigint, text, bigint, timestamptz, text, text, uuid, inet, text) to service_role;
grant execute on function public.m04_ads_transition_campaign_plan(bigint, bigint, text, text, text, uuid, inet, text) to service_role;
grant execute on function public.m04_ads_acquire_campaign_gate_claim(bigint, smallint, text, text, bigint, text, integer, jsonb, uuid, inet, text) to service_role;
grant execute on function public.m04_ads_acquire_campaign_retry_claim(bigint, bigint, text, text, integer, jsonb, uuid, inet, text) to service_role;
grant execute on function public.m04_ads_record_campaign_resource_outcome(bigint, uuid, text, text, text, text, jsonb, jsonb) to service_role;
grant execute on function public.m04_ads_append_campaign_qa_result(bigint, uuid, bigint, text, text, boolean, jsonb, jsonb, text, text, text, jsonb) to service_role;
grant execute on function public.m04_ads_finalize_campaign_gate_claim(bigint, uuid, text, jsonb, uuid, inet, text) to service_role;
grant execute on function public.m04_ads_create_campaign_monitoring_handoff(bigint, bigint, text, uuid, inet, text) to service_role;
...
drop index public.m04_acga_build_gate_active_idx;
create unique index m04_acga_build_active_idx on public.m04_ads_campaign_gate_attempts(build_id)
  where released_at is null and status = 'claimed';
...
begin
  ...
  from m04_ads_internal.acquire_campaign_gate_claim_hardened(
...
begin
  select * into strict v_attempt
  from m04_ads_internal.acquire_campaign_retry_claim_hardened(
...
revoke all on function m04_ads_internal.acquire_campaign_gate_claim_hardened(
  bigint, smallint, text, text, bigint, text, integer, jsonb, uuid, inet, text
) from public, anon, authenticated, service_role;
revoke all on function m04_ads_internal.acquire_campaign_retry_claim_hardened(
  bigint, bigint, text, text, integer, jsonb, uuid, inet, text
) from public, anon, authenticated, service_role;

create or replace function m04_ads_internal.campaign_gate_1_full_intent_ready(
  p_build_id bigint,
    ...
    left join lateral (
      select pg_catalog.count(*) > 0 as has_required_fields,
        pg_catalog.bool_and(
          ...
          and m04_ads_internal.is_nonempty_json_object(field_evidence.readback_evidence)
        ) as all_required_fields_match
      from (
        select distinct on (qa.field_path)
          qa.field_path, qa.result, qa.readback_evidence
        from public.m04_ads_campaign_qa_results as qa
        join public.m04_ads_campaign_gate_attempts as evidence_attempt
...
$$;
revoke all on function m04_ads_internal.campaign_gate_1_full_intent_ready(bigint, bigint)
  from public, anon, authenticated, service_role;
...
begin
  ...
  if v_attempt.gate = 1 and v_attempt.action = 'reconcile' then
    select not exists (
      ...
      left join lateral (
        select pg_catalog.count(*) > 0 as has_required_fields,
          pg_catalog.bool_and(
            m04_ads_internal.is_nonempty_json_object(field_qa.readback_evidence)
    ...
    select m04_ads_internal.campaign_gate_1_full_intent_ready(v_build.id, v_attempt.id)
        ...
        left join lateral (
          select pg_catalog.count(*) > 0 as has_required_fields,
            pg_catalog.bool_and(
              ...
              and m04_ads_internal.is_nonempty_json_object(field_qa.readback_evidence)
            ) as all_required_fields_match
          from (
            select distinct on (qa.field_path)
              qa.field_path, qa.result, qa.readback_evidence
    ...
    if p_provider_outcome = 'succeeded' then
      select m04_ads_internal.is_nonempty_json_object(p_final_readback_evidence)
        and m04_ads_internal.campaign_gate_2_manifest_ready(
  ...
  set status = v_to_status,
      ...
      verified_at = case when v_to_status = 'verified' then v_verified_at else verified_at end,
      final_readback_evidence = case
        when v_attempt.gate = 2 and m04_ads_internal.is_nonempty_json_object(p_final_readback_evidence)
          then p_final_readback_evidence
        when v_attempt.gate = 2 then '{}'::jsonb
        else final_readback_evidence
      end,
...
declare
  ...
  v_account public.m04_ads_ad_accounts%rowtype;
  v_package public.m04_ads_budget_packages%rowtype;
  v_current_approval public.m04_ads_campaign_approvals%rowtype;
  v_existing_approval public.m04_ads_campaign_approvals%rowtype;
  v_new_approval public.m04_ads_campaign_approvals%rowtype;
  v_build public.m04_ads_campaign_builds%rowtype;
  v_actor_name text;
  v_actor_email text;
  v_actor_role text;
...
begin
  ...
  from public.m04_ads_campaign_plans as plan where plan.id = p_plan_id;
  if not found then
    raise exception 'Campaign plan was not found' using errcode = 'P0002';
  end if;
  ...
  if v_pre_status in ('approved', 'launch_in_progress') then
    ...
    select build.* into strict v_build
    from public.m04_ads_campaign_builds as build where build.id = v_pre_build_id for update;
    select plan.* into strict v_plan
    from public.m04_ads_campaign_plans as plan where plan.id = p_plan_id for update;
    ...
    from public.m04_ads_budget_packages as package where package.id = v_plan.budget_package_id;
    select account.* into strict v_account
    from public.m04_ads_ad_accounts as account where account.id = v_plan.ad_account_id;
  ...
  if found then
    ...
    select build.* into strict v_build from public.m04_ads_campaign_builds as build
  ...
  select package.* into strict v_package
  from public.m04_ads_budget_packages as package where package.id = v_plan.budget_package_id for update;
  select account.* into strict v_account
  from public.m04_ads_ad_accounts as account where account.id = v_plan.ad_account_id;
  if not v_account.is_active or v_account.access_status <> 'verified'
    or v_account.access_verified_at is null then
...
create table public.m04_ads_google_campaign_revision_details (
  revision_id bigint primary key
    constraint acgrd_revision_fk
    references public.m04_ads_campaign_plan_revisions(id) on delete restrict,
...
create table public.m04_ads_meta_campaign_revision_details (
  revision_id bigint primary key
...
create table public.m04_ads_tiktok_campaign_revision_details (
  revision_id bigint primary key
...
before update or delete on public.m04_ads_google_campaign_revision_details
for each row execute function m04_ads_internal.reject_m04_row_mutation();

create trigger acmrd_append_only
before update or delete on public.m04_ads_meta_campaign_revision_details
for each row execute function m04_ads_internal.reject_m04_row_mutation();

create trigger actrd_append_only
before update or delete on public.m04_ads_tiktok_campaign_revision_details
for each row execute function m04_ads_internal.reject_m04_row_mutation();

alter table public.m04_ads_google_campaign_revision_details enable row level security;
alter table public.m04_ads_meta_campaign_revision_details enable row level security;
alter table public.m04_ads_tiktok_campaign_revision_details enable row level security;

revoke all on table public.m04_ads_google_campaign_revision_details
  from public, anon, authenticated, service_role;
revoke all on table public.m04_ads_meta_campaign_revision_details
  from public, anon, authenticated, service_role;
revoke all on table public.m04_ads_tiktok_campaign_revision_details
  from public, anon, authenticated, service_role;

grant select on table public.m04_ads_google_campaign_revision_details to service_role;
grant select on table public.m04_ads_meta_campaign_revision_details to service_role;
grant select on table public.m04_ads_tiktok_campaign_revision_details to service_role;

create or replace function public.m04_ads_create_campaign_plan_draft(
  p_client_id uuid,
...
begin
  ...
  insert into public.m04_ads_campaign_plans (
    client_id,
    budget_package_id,
    ad_account_id,
    platform,
  ...
  perform m04_ads_internal.append_campaign_audit(
    ...
    pg_catalog.jsonb_build_object(
      ...
      'platform_detail_table', case p_platform
        when 'google' then 'm04_ads_google_campaign_revision_details'
        when 'meta' then 'm04_ads_meta_campaign_revision_details'
        else 'm04_ads_tiktok_campaign_revision_details'
      end
    )
  );
...
revoke all on function public.m04_ads_create_campaign_plan_draft(
  uuid, bigint, bigint, text, jsonb, text, text, jsonb, uuid, inet, text
) from public, anon, authenticated, service_role;

grant execute on function public.m04_ads_create_campaign_plan_draft(
  uuid, bigint, bigint, text, jsonb, text, text, jsonb, uuid, inet, text
) to service_role;
